...
* HLA/IMGT Database Release: 3.25.0, 2016-07-14
...
Data output to both the console and text files can be supressed with Verbose=F and Output=F respectively. This is particularly useful when the user prefers to send the results to an object (Return=T) rather than to text files for further analysis. The result is returned as a list with indices for the Hardy-Weinberg equilibrium test (HWE when run) and indices corresponding to the results for each Loci.Set (Set1, Set2, etc). If no Loci.Sets were specified, then only one list element will exist for Set1.
...
..\$Set1\$H - Results of the 'Haplotype' analysis for Set1  
..\$Set1\$L - Results of the 'Locus' analysis for Set1  
..\$Set1\$A - Results of the 'Amino Acid' analysis for Set1  
  
 names(BD) - Display complete list of available sets to index  
  
If the user chooses to run all pairwise combinations of loci (All.Pairwise=T) for the haplotype analysis, BD\$H mentioned above will be a list corresponding each analysis run for all loci available (1st index in the list) and for each pairwise locus combination. For a complete list of available pairwise results, use names(BD\$H).
...
`BIGDAWG(Data, HLA=T, Run.Tests, Loci.Set, All.Pairwise=F, Trim=F, Res=2, Missing=0, Cores.Lim=1L, Results.Dir, Return=F, Output=T, Verbose=T)`
...
Input list defining which loci to use for analyses. Combinations are permitted. If you included HLA-DRB3,-DRB4,-DRB5 as a collapsed column pair ('DRB3/4/5'), you must specifiy the single locus in the Loci.set if you wish them to be included in an analysis set (i.e., 'DRB3' **NOT** 'DRB3/4/5').

The pair of alleles for a locus must be in adjacent columns in the analyzed data set. Running multiple sets is ONLY relevant for the haplotype analysis. For all other analyses, loci are treated independently. Consider running haplotype analysis independently when optioning multi-locus sets to avoid redundancy of the other analyses. Each locus set's output will be contained within a separate set folder numbered numerical (see Data Output section).
...
Developmental versions of BIGDAWG can be downloaded through [GitHub](https://github.com/pappasd/BIGDAWG/) or using the following code (requires R package 'devtools'). These versions will include the most up-to-date bug fixes as well as access to new features that are still under development. GitHub versions are constantly under development and if you prefer a more stable fixed release, install BIGDAWG from the CRAN respository. You may check BIGDAWG versions using CheckRelease().
...
library("devtools")
install_github("pappasd/BIGDAWG", build_vignettes = TRUE)

# For a temporarily install of the developmental version
library("devtools")
dev_mode(on=T)
install_github("pappasd/BIGDAWG")

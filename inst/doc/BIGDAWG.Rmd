...
**Novel Alleles**  
BIGDAWG will accept novel allele names. However, it is suggested you follow the same naming convention for novel alleles as with other genotypes calls in your data, either with or without the locus prefix. For example, new alleles could be submitted as follows: New, 01:New, or A*01:New. Unfortunately, the BIGDAWG amino acids analysis cannot accept new allele designations and will display an error. If you would like to run the amino acid analysis, it is suggested you replace the new allele with NA or omit the subject entirely.  
...
Data output to both the console and text files can be supressed with Verbose=F and Output=F respectively. This is particularly useful when the user prefers to send the results to an object (Return=T) rather than to text files for further analysis. The result is returned as a list with indices for the Hardy-Weinberg equilibrium test and indices for each result within the Loci.Set (Set1, Set2, etc). If no Loci.Sets were specified, only one list element will exist for Set1.
...
If the user chooses to run all pairwise combinations of loci (All.Pairwise=T) for the haplotype analysis, BD\$H mentioned above will be a list corresponding to each analysis run for (1) all loci available (1st index in the list) and (2) for each pairwise locus combination (2nd index and greater). For a complete list of available pairwise results, use names(BD\$H).
